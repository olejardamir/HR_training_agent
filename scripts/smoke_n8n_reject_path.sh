...
echo "=== Smoke: n8n Webhook Reject Path ==="
...
echo "2. Post to webhook /webhook/hr-onboarding/new-hire (reject path)"
...
assert d.get('ticket_created') is False, 'ticket_created must be false on reject'
assert d.get('ticket_id') is None, f'ticket_id must be null on reject, got {d.get(\"ticket_id\")!r}'
assert d.get('demo_status') == 'COMPLETED_NO_TICKET', f'expected COMPLETED_NO_TICKET, got {d.get(\"demo_status\")}'
assert d.get('approval_status') == 'REJECTED', f'expected REJECTED, got {d.get(\"approval_status\")}'
print(f'   ok=true, approval_status=REJECTED, ticket_created=false')
print(f'   demo_status=COMPLETED_NO_TICKET')
"

echo "=== N8N WEBHOOK REJECT SMOKE PASSED ==="
