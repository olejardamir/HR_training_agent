...
echo "=== Smoke: n8n Webhook Pending Path ==="
...
echo "2. Post to webhook /webhook/hr-onboarding/new-hire (pending path)"
...
assert d.get('ticket_created') is False, 'ticket_created must be false on pending'
assert d.get('ticket_id') is None, f'ticket_id must be null on pending, got {d.get(\"ticket_id\")!r}'
assert d.get('demo_status') == 'COMPLETED_NO_TICKET', f'expected COMPLETED_NO_TICKET, got {d.get(\"demo_status\")}'
assert d.get('approval_status') == 'PENDING', f'expected PENDING, got {d.get(\"approval_status\")}'
print(f'   ok=true, approval_status=PENDING, ticket_created=false')
print(f'   demo_status=COMPLETED_NO_TICKET')
"

echo "=== N8N WEBHOOK PENDING SMOKE PASSED ==="
